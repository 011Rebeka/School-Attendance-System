...
$db_sinfo=mysqli_query($link,"SELECT * FROM `teachers` WHERE `contact`='$contact' AND `class_teacher`='1'");
  ...
  while($row = mysqli_fetch_assoc($db_sinfo)) { ?>
			...
			<tr>
				...
				<td><a class="btn btn-xs btn-warning" href="edit_student_zone.php?id=<?php echo base64_encode($row['id']); ?>"><i class="fa fa-pencil"></i>Edit</a>
					<a onClick="javascript: return confirm('Please confirm deletion: <?php echo ucwords($row["name"]);?>');" class="btn btn-xs btn-danger" href="delete_student_zone.php?id=<?php echo base64_encode($row['id']); ?>"><i class="fa fa-trash"></i>Delete</a>
				</td>
  ...
  <?php } ?>
	  ...
	  <?php  }
else {echo '<h1>You Are Not Class Teacher.</h1>';}	  
  ?>
